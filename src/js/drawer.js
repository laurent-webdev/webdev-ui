export default class Drawer {
    ...
    static onClick(event) {
        // FIX : closest au lieu de matches pour couvrir les clics sur enfants éventuels
        const backdrop = event.target.closest('.drawer-backdrop');
        if (backdrop) {
            const drawerId = backdrop.dataset.for;
            const drawer = document.getElementById(drawerId);
            const button = drawer ? document.querySelector(`[data-ui="drawer"][data-target="#${drawerId}"]`) : null;
            if (drawer && button) Drawer.close(drawer, button);
            return;
        }
    ...
    static updateTriggers(drawer, expanded) {
        document.querySelectorAll(`[data-ui="drawer"][data-target="#${drawer.id}"]`)
    ...
    static destroy() {
        // Nettoyer les backdrops et états ouverts
